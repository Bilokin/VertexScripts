...
void probability(){
	...
	for (unsigned int i = 0; i < mTotalNumberOfEvents; i += 1)
	{
		...
		if (secondaryb > -1 || secondarybbar > -1) 
		{
			if (secondaryb > -1 && ternaryb > -1) 
			{
				probhist->Fill(_probability[secondaryb]);
				chihist->Fill(_chi2[secondaryb]);

			}
			if (secondarybbar > -1 && ternarybbar > -1) 
			{
				probhist->Fill(_probability[secondarybbar]);
				chihist->Fill(_chi2[secondarybbar]);
			}
			if (secondaryb > -1 && ternaryb < 0) 
			{
				probhist_non->Fill(_probability[secondaryb]);
				chihist_non->Fill(_chi2[secondaryb]);
			}
			if (secondarybbar > -1 && ternarybbar < 0) 
			{
				probhist_non->Fill(_probability[secondarybbar]);
				chihist_non->Fill(_chi2[secondarybbar]);
			}
			counter++;
		}
	}
