...
#include <iostream>
#define MAXV 8
void statistics(string mc = "TrashMCTest.root", string reco = "TrashRecoTest.root")
{
	int bin_e = 50;
	int max_e = 1;
	TFile * mcfile = TFile::Open(mc.c_str());
	mcfile->cd();
	TH1F * counthist = new TH1F("cosOne", "E(Ntracks)", 100,0.0,100);
	Stats->Project("btotalnumber >> counthist")
	TFile * recofile = TFile::Open(reco.c_str());
